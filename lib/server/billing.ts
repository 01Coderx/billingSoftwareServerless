...
async function nextSequence(
  ...
  const highestQuery = Model.findOne({})
    .sort({ id: -1 })
    .select({ id: 1 });

  if (session) {
    highestQuery.session(session);
  }
...
export async function getProduct(id: string | number) {
  ...
  if (product) {
    await setCacheSafe(key, product, 300);
  }
    ...
    await session.withTransaction(async () => {
      ...
      for (const raw of input.items) {
        ...
        if (!Number.isFinite(productId)) {
          throw new Error(
            "Every stock item must have a valid product"
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Purchase rate must be zero or greater"
          );
        }
...
async function buildInvoice(
  ...
  if (input.customer?.id != null) {
    ...
    const found =
      await customerQuery.lean<any>();

    if (!found) {
      throw new Error(
        `Customer not found: ${input.customer.id}`
      );
    }
...
export async function createInvoice(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Invoice stock protection requires a MongoDB deployment with transaction support (replica set/Atlas). No invoice was created."
      );
    }
  ...
  try {
    ...
    await session.withTransaction(
      async () => {
        ...
        const oldMap =
          stockMap(existing.items);

        const newMap =
          stockMap(built.items);
        ...
        if (
          ...
          if (changed) {
            throw new Error(
              "This older invoice has no inventory history. Quantity changes are blocked to protect stock accuracy."
            );
          }
        } else {
          const productIds =
  ...
  try {
    ...
    await session.withTransaction(
      async () => {
        ...
        if (amount > due) {
          throw new Error(
            `Payment exceeds outstanding amount of ₹${due.toFixed(
              2
            )}`
          );
        }
...
export async function getDashboardAnalytics() {
   ...
   * IMPORTANT:
   *
   * This is MONTHLY analytics.
   * Do not put the topProducts aggregation here.
   */
...
export async function getReportsAnalytics() {
  ...
  const [
    ...
    /* -----------------------------------------------------
       ...
       IMPORTANT:
       Product name + SKU come from the CURRENT
       Product collection.

       This prevents old invoice snapshots such
       as "Mini Mogra" from being shown when the
       current product is "PK DAL".
    ----------------------------------------------------- */
...
export async function getSettings() {
  return BusinessSettings.findOne({
    key: "default",
  }).lean();
}

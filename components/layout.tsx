...
function Sidebar({ close }: { close?: () => void }) {
  ...
  return <aside className="flex h-full w-[260px] flex-col bg-[var(--sidebar)] text-white">
    ...
    <div className="flex-1 p-4">
      ...
      <Link href="/reports" onClick={close} className={cn("flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition", pathname === "/reports" || pathname.startsWith("/reports/") ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5 hover:text-white")}><BarChart3 size={18}/>Reports</Link>
    </div>

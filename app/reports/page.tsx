...
export default function ReportsPage() {
  ...
  const recurring = data?.recurringCustomers || [];
  ...
  return (
    <div className="fade-in">
      ...
      <div className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Products" value={Number(products.products || 0).toLocaleString("en-IN")} sub={`${Number(products.unitsInStock || 0).toLocaleString("en-IN")} units in stock`} icon={Boxes} />
        <MetricCard label="Inventory value" value={formatCurrency(products.inventoryValue)} sub="Cost value of current stock" icon={IndianRupee} tone="emerald" />
        <MetricCard label="Recurring customers" value={Number(customers.recurringCustomers || 0).toLocaleString("en-IN")} sub={`${Number(customers.customers || 0).toLocaleString("en-IN")} customers with sales`} icon={UserRoundCheck} tone="violet" />
        <MetricCard label="Products to restock" value={Number(products.lowStock || 0).toLocaleString("en-IN")} sub={`At or below ${products.lowStockThreshold ?? 5} units`} icon={AlertTriangle} tone="amber" />
      </div>
        ...
        <Section title="Inventory Snapshot" description="Products that need attention." icon={PackageCheck} action={<span className="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-black text-amber-700">Target: {products.targetStock ?? 10}</span>}>
          <div className="p-5">
            <div className="mb-4 grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-bold text-slate-500">Units left</div><div className="mt-1 text-xl font-black">{Number(products.unitsInStock || 0).toLocaleString("en-IN")}</div></div>
            ...
            <div className="space-y-2">
              {lowStock.length ? lowStock.map((product: any) => (
                <div key={product.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 p-3">
                  ...
                  <div className="shrink-0 text-right"><div className="text-xs font-black text-rose-600">{product.stock} left</div><div className="text-[10px] font-bold text-slate-400">{product.required} required</div></div>
                </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        ...
        <Section title="Customer Analysis" description="Customer sales, repeat behaviour and outstanding amounts." icon={Users}>
          <div className="grid gap-5 p-5 sm:grid-cols-[150px_1fr] sm:items-center">
            <div className="h-36">
              {customers.customers ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={customerMix} dataKey="value" nameKey="name" innerRadius={42} outerRadius={60} paddingAngle={4}>{customerMix.map((_: any, i: number) => <Cell key={i} fill={i === 0 ? "#2563eb" : "#cbd5e1"} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer> : <div className="grid h-full place-items-center text-xs text-slate-400">No data</div>}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-bold text-blue-700">Total customers</div><div className="mt-1 text-xl font-black text-blue-950">{Number(customers.customers || 0)}</div></div>
              <div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-bold text-emerald-700">Customer sales</div><div className="mt-1 text-xl font-black text-emerald-950">{formatCurrency(customers.sales)}</div></div>
              ...
              <div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-bold text-amber-700">Outstanding</div><div className="mt-1 text-xl font-black text-amber-950">{formatCurrency(customers.outstanding)}</div></div>
            </div>
          </div>
        </Section>
      </div>
        ...
        <Section title="Recurring Customers" description="Customers with two or more non-cancelled invoices." icon={UserRoundCheck} action={<span className="text-xs font-bold text-slate-500">{recurring.length} shown</span>}>
          <div className="divide-y divide-slate-100">
            {recurring.length ? recurring.map((customer: any) => (
              <div key={customer.id} className="flex items-center justify-between gap-4 p-4 sm:px-5"><div className="min-w-0"><div className="truncate font-bold">{customer.name}</div><div className="text-xs text-slate-500">{customer.invoices} invoices · {customer.phone || customer.email || "No contact"}</div></div><div className="shrink-0 text-right"><div className="font-black">{formatCurrency(customer.sales)}</div><div className="text-[11px] text-slate-500">total sales</div></div></div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.4fr_.6fr]">
        <Section title="Sales Trend" description="Monthly invoice sales from your live database." icon={BarChart3}>
          <div className="h-[300px] p-4 sm:p-5">
            {(data?.monthlySales || []).length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={data.monthlySales}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="label"/><YAxis tickFormatter={(value) => `₹${Number(value).toLocaleString("en-IN")}`} /><Tooltip formatter={moneyTooltip}/><Bar dataKey="sales" name="Sales" fill="#2563eb" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer> : <div className="grid h-full place-items-center text-sm text-slate-500">No sales trend yet.</div>}
          </div>
        </Section>
